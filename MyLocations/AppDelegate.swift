...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(application: UIApplication, didFinishLaunchingWithOptions launchOptions: [NSObject: AnyObject]?) -> Bool {
        ...
        if let tabBarControllers = tabBarController.viewControllers {
            ...
            let locationsController = navigationController.viewControllers[0] as! LocationsViewController;
            
            locationsController.managedObjectContext = managedObjectContext;
            let _ = locationsController.view; // Soulution to Core Data bug
    ...
    lazy var persistentStoreCoordinator: NSPersistentStoreCoordinator = {
        ...
        let url = applicationDocumentsDirectory.URLByAppendingPathComponent("Locations.sqlite")
    ...
    func listenForFatalCoreDataNotifications() {
       //1
        NSNotificationCenter.defaultCenter().addObserverForName("MyManagedObjectContextSaveDidFailNotification", object: nil, queue: NSOperationQueue.mainQueue(), usingBlock: {notification in
            ...
            //2
            let alert = UIAlertController(title: "Internal error", message: "There are was a fatal error in the app and it cannot continue. \n\n" +
                "Press OK to terminate the app. Sorry for inconvenience.", preferredStyle: .Alert);
        
            //3
            let action = UIAlertAction(title: "OK", style: .Default) {_ in
                let exeption = NSException(name: "InternalInconsistencyExeption", reason: "Fatal Core Data Error", userInfo: nil);
            exeption.raise()
